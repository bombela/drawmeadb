/* $id: shapes.c,v 1.82 2007/12/24 04:50:36 ellson Exp $ $Revision: 1.83 $ */
...
parse_reclbl(node_t * n, int LR, int flag, char *text)
{
	...
	case '\000':
		...
		*tsp = '\000';
		...
		// Ugly hack by Bombela

		fp->nexttopline = bottomline;
		fp->topline = lastbottomline;
...
static void gen_fields(GVJ_t * job, node_t * n, field_t * f)
{
	...
	topline = f->fld[i]->topline;
	botline = f->fld[i]->nexttopline;
